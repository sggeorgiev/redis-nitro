 ...
 * data structure.
 *
 * The elements are added to a hash table mapping each member (an sds string)
 * to its score (a double stored in the dict entry value), giving O(1) point
 * lookups. At the same time the elements are added to a DASL (an array-packed
 * skip list, see dasl.c/dasl.h) keyed by (score, member), so members are
 * sorted by score in that "view" with O(log N) ordered access.
 *
 * Note that the SDS string representing the member is shared between both
 * structures to save memory: the dict key is the very same buffer the DASL
 * owns in its bottom-level slot. The DASL is the sole owner and frees the sds
 * exactly once on element removal. Because of this the dict (which has no key
 * destructor) must always be updated first - unlink the dict entry, then delete
 * from the DASL, which is what finally releases the shared sds. */
#include "fast_float_strtod.h"
#include "server.h"
#include "intset.h"  /* Compact integer set structure */
#include <math.h>

/* dictType for zset's dict: maps a member sds (the key, a pointer shared with
 * the DASL which owns the buffer) to its score (a double in the entry value).
 * There is no key destructor because the DASL owns and frees the member sds. */
dictType zsetDictType = {
    dictSdsHash,        /* hash function */
    NULL,               /* key dup */
    NULL,               /* val dup */
    dictSdsKeyCompare,  /* key compare */
    NULL,               /* key destructor - the DASL owns the member sds */
    NULL,               /* val destructor - score is stored inline as a double */
    NULL,               /* allow to expand */
};

/*-----------------------------------------------------------------------------
 * Score / lexicographic range helpers (shared by the DASL and listpack views)
 *----------------------------------------------------------------------------*/
...
/* Insert (score, ele) into a skiplist-encoded zset: add to the DASL (which
 * copies and owns the member sds) and to the dict, where the key is that same
 * shared buffer and the value is the score. The caller keeps ownership of
 * `ele`. The element must not already be present. */
static inline void zsetSkiplistInsert(zset *zs, double score, sds ele) {
    daslCursor c = daslInsert(zs->dasl, score, ele);
    sds owned = daslCursorMember(&c);
    dictEntry *de = dictAddRaw(zs->dict, owned, NULL);
...
unsigned long zsetLength(const robj *zobj) {
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        length = daslLength(((const zset*)zobj->ptr)->dasl);
    } else {
...
size_t zsetAllocSize(const robj *o) {
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        dasl *sl = ((zset*)o->ptr)->dasl;
        size = sizeof(zset) + daslAllocSize(sl) +
...
void zsetConvertAndExpand(robj *zobj, int encoding, unsigned long cap) {
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        while (eptr != NULL) {
            ...
            /* The DASL copies the member and owns the buffer; the dict key is
             * that same shared buffer, with the score stored as the value. */
            daslCursor c = daslInsert(zs->dasl,score,ele);
            sds owned = daslCursorMember(&c);
            dictEntry *de = dictAddRaw(zs->dict, owned, NULL);
            serverAssert(de != NULL);
            dictSetDoubleVal(de, score);
            sdsfree(ele); /* daslInsert copied it, we can free our copy */
        ...
        /* The dict has no key destructor (the DASL owns the member sds), so
         * releasing it frees only the entries; daslFree releases the members. */
        zs = zobj->ptr;
        dictRelease(zs->dict);

        for (daslCursor c = daslFirst(zs->dasl); !daslCursorIsNull(&c); c = daslNext(c))
            zl = zzlInsertAt(zl,NULL,daslCursorMember(&c),daslCursorScore(&c));
...
void zsetConvertToListpackIfNeeded(robj *zobj, size_t maxelelen, size_t totelelen) {
    ...
    if (daslLength(zset->dasl) <= server.zset_max_listpack_entries &&
    ...
    *out_flags = 0; /* We'll return our response flags. */
    ...
    if (zobj->encoding == OBJ_ENCODING_SKIPLIST) {
        ...
        if (link != NULL) {
            ...
            /* The score is stored as the dict entry value. */
            ...
            /* Remove and re-insert when score changes. The member sds is the
             * dict key, which is the very buffer the DASL owns; daslReposition
             * preserves that buffer so the dict key stays valid, and we just
             * refresh the stored score. */
            if (score != curscore) {
                sds member = dictGetKey(de);
                daslReposition(zs->dasl, curscore, score, member);
                dictSetDoubleVal(de, score);
                *out_flags |= ZADD_OUT_UPDATED;
            }
            return 1;
...
static int zsetRemoveFromSkiplist(zset *zs, sds ele) {
    ...
    if (de != NULL) {
        /* Capture the score (entry value) and the shared member buffer (the
         * dict key, owned by the DASL) before releasing the entry. */
        double score = dictGetDoubleVal(de);
        sds member = dictGetKey(de);

        /* Order is important: free the (key-destructor-less) dict entry first,
         * then delete from the DASL, which is what actually releases the shared
         * member sds. */
        dictFreeUnlinkedEntry(zs->dict,de);

        /* Delete from the DASL (frees the member sds). */
        daslDelete(zs->dasl, score, member);
...
long zsetRank(robj *zobj, sds ele, int reverse, double *output_score) {
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        if (de != NULL) {
            double score = dictGetDoubleVal(de);
            sds member = dictGetKey(de);
            rank = daslGetRank(zs->dasl, score, member);
...
robj *zsetDup(robj *o) {
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        /* Copy from the greatest element to the smallest (the DASL is already
         * ordered). The member sds inserted into the new DASL is owned there;
         * the new dict shares that buffer as its key, with the score as its
         * value. */
        daslCursor ln = daslLast(zs->dasl);
        while (llen--) {
            double score = daslCursorScore(&ln);
            sds ele = daslCursorMember(&ln);
            daslCursor nc = daslInsert(new_zs->dasl,score,ele);
            sds owned = daslCursorMember(&nc);
            dictEntry *de = dictAddRaw(new_zs->dict, owned, NULL);
            serverAssert(de != NULL);
            dictSetDoubleVal(de, score);
            ln = daslPrev(ln);
        }
...
void zremrangeGenericCommand(client *c, zrange_type rangetype) {
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        case ZRANGE_AUTO:
        case ZRANGE_RANK:
            deleted = daslDeleteRangeByRank(zs->dasl,start+1,end+1,zs->dict);
            break;
        case ZRANGE_SCORE:
            deleted = daslDeleteRangeByScore(zs->dasl,&range,zs->dict);
            break;
        case ZRANGE_LEX:
            deleted = daslDeleteRangeByLex(zs->dasl,&lexrange,zs->dict);
            break;
        }
...
void zuiInitIterator(zsetopsrc *op) {
    ...
    if (op->type == OBJ_SET) {
        ...
        if (op->encoding == OBJ_ENCODING_LISTPACK) {
            ...
            it->sl.node = daslLast(it->sl.zs->dasl);
        } else {
            serverPanic("Unknown sorted set encoding");
        }
    } else {
...
int zuiNext(zsetopsrc *op, zsetopval *val) {
    ...
    if (op->type == OBJ_SET) {
        ...
        if (op->encoding == OBJ_ENCODING_LISTPACK) {
            ...
            if (daslCursorIsNull(&it->sl.node))
                return 0;
            val->ele = daslCursorMember(&it->sl.node);
            val->score = daslCursorScore(&it->sl.node);

            /* Move to next element. (going backwards, see zuiInitIterator) */
            it->sl.node = daslPrev(it->sl.node);
        } else {
            serverPanic("Unknown sorted set encoding");
        }
    } else {
...
static size_t zsetDictGetMaxElementLength(dict *d, size_t *totallen) {
    ...
    while((de = dictNext(&di)) != NULL) {
        /* The dict key is the member sds (shared with the DASL). */
...
static void zdiffAlgorithm1(zsetopsrc *src, long setnum, zset *dstzset, size_t *maxelelen, size_t *totelelen) {
    ...
    while (zuiNext(&src[0],&zval)) {
        ...
        if (!exists) {
            ...
            zsetSkiplistInsert(dstzset,zval.score,tmp);
            if (sdslen(tmp) > *maxelelen) *maxelelen = sdslen(tmp);
            (*totelelen) += sdslen(tmp);
            sdsfree(tmp); /* the DASL copied it, we can free our copy */
        }
    }
...
void zunionInterDiffGenericCommand(client *c, robj *dstkey, int numkeysIndex, int op,
    ...
    if (op == SET_OP_INTER) {
        ...
        if (zuiLength(&src[0]) > 0) {
            ...
            while (zuiNext(&src[0],&zval)) {
                ...
                if (j == setnum && cardinality_only) {
                    ...
                    tmp = zuiNewSdsFromValue(&zval);
                    zsetSkiplistInsert(dstzset,score,tmp);
                    totelelen += sdslen(tmp);
                    if (sdslen(tmp) > maxelelen) maxelelen = sdslen(tmp);
                    sdsfree(tmp); /* the DASL copied it, we can free our copy */
                }
        ...
        /* Iterate all sorted sets and aggregate scores. For each element,
         * either insert it (new) or aggregate onto the existing score and
         * reposition it in the DASL. The dict key is the DASL-owned member sds
         * and the dict value holds the running aggregated score. */
        for (i = 0; i < setnum; i++) {
            ...
            while (zuiNext(&src[i],&zval)) {
                ...
                dictEntry *fde = dictFind(dstzset->dict, zuiSdsFromValue(&zval));

                if (fde == NULL) {  /* if not exists */
                    /* New element: insert into the DASL and dict. */
                    tmp = zuiNewSdsFromValue(&zval);
                    ...
                    /* Existing element: aggregate score and reposition. */
                    double oldscore = dictGetDoubleVal(fde);
                    double newscore = oldscore;
                    zunionInterAggregate(&newscore, score, aggregate);
                    if (newscore != oldscore) {
                        sds member = dictGetKey(fde);
                        daslReposition(dstzset->dasl, oldscore, newscore, member);
                        dictSetDoubleVal(fde, newscore);
                    }
                }
            }
    ...
    if (dstkey) {
        ...
        unsigned long length = daslLength(dstzset->dasl);
        daslCursor zn = daslFirst(dstzset->dasl);
        ...
        while (!daslCursorIsNull(&zn)) {
            if (withscores && c->resp > 2) addReplyArrayLen(c,2);
            sds ele = daslCursorMember(&zn); addReplyBulkCBuffer(c,ele,sdslen(ele));
            if (withscores) addReplyDouble(c,daslCursorScore(&zn));
            zn = daslNext(zn);
        }
...
void genericZrangebyrankCommand(zrange_result_handler *handler,
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        if (reverse) {
            ln = daslLast(sl);
            if (start > 0)
                ln = daslGetElementByRank(sl,llen-start);
        } else {
            ln = daslFirst(sl);
            if (start > 0)
        ...
        while(rangelen--) {
            serverAssertWithInfo(c,zobj,!daslCursorIsNull(&ln));
            sds ele = daslCursorMember(&ln);
            handler->emitResultFromCBuffer(handler, ele, sdslen(ele), daslCursorScore(&ln));
            ln = reverse ? daslPrev(ln) : daslNext(ln);
        }
    } else {
        serverPanic("Unknown sorted set encoding");
    }
...
void genericZrangebyscoreCommand(zrange_result_handler *handler,
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        /* If reversed, get the last node in range as starting point. */
        if (reverse) {
            ln = daslNthInRange(sl, range, -offset-1, NULL);
        } else {
            ln = daslNthInRange(sl, range, offset, NULL);
        }

        while (!daslCursorIsNull(&ln) && limit--) {
            double score = daslCursorScore(&ln);
            /* Abort when the node is no longer in range. */
            if (reverse) {
                if (!zslValueGteMin(score,range)) break;
...
void zcountCommand(client *c) {
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        daslCursor zn;
        unsigned long rank;
        unsigned long len = daslLength(sl);

        /* Find first element in range and get its rank */
        zn = daslNthInRange(sl, &range, 0, &rank);

        /* Use rank of first element, if any, to determine preliminary count */
        if (!daslCursorIsNull(&zn)) {
            count = (len - (rank - 1));

            /* Find last element in range and get its rank */
            zn = daslNthInRange(sl, &range, -1, &rank);
...
void zlexcountCommand(client *c) {
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        /* Find first element in range and get its rank */
        zn = daslNthInLexRange(sl, &range, 0, &rank);
        ...
        if (!daslCursorIsNull(&zn)) {
            ...
            /* Find last element in range and get its rank */
            zn = daslNthInLexRange(sl, &range, -1, &rank);
    ...
    int reverse)
{
    ...
    if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        /* If reversed, get the last node in range as starting point. */
        if (reverse) {
            ln = daslNthInLexRange(sl,range,-offset-1,NULL);
        } else {
            ln = daslNthInLexRange(sl,range,offset,NULL);
        }
...
void genericZpopCommand(client *c, robj **keyv, int keyc, int where, int emitkey,
    ...
    do {
        if (zobj->encoding == OBJ_ENCODING_LISTPACK) {
            ...
            /* Get the first or last element in the sorted set. */
            daslCursor zln = (where == ZSET_MAX ? daslLast(zs->dasl) :
                                                  daslFirst(zs->dasl));

            /* There must be an element in the sorted set. */
            serverAssertWithInfo(c,zobj,!daslCursorIsNull(&zln));
            ele = sdsdup(daslCursorMember(&zln));
            score = daslCursorScore(&zln);
        } else {
            serverPanic("Unknown sorted set encoding");
        }
...
int zsetTest(int argc, char **argv, int flags) {
    ...
    printf("Testing DASL (sorted set ordered index) operations\n");
    ...
    /* Insert N elements with random scores between 0 and 100. Element names are
     * unique so (score, ele) pairs are always distinct. */
    for (int i = 0; i < N; i++) {
        ...
        daslCursor c = daslInsert(sl, score, ele);
        assert(!daslCursorIsNull(&c));

        elements[i].score = score;
        elements[i].ele = ele;

        /* Query the inserted element. */
        unsigned long rank = daslGetRank(sl, score, ele);
        assert(rank != 0);

        /* Verify we can get the element back by rank. */
        daslCursor found = daslGetElementByRank(sl, rank);
        assert(!daslCursorIsNull(&found));
        assert(sdscmp(daslCursorMember(&found), ele) == 0);
        assert(daslCursorScore(&found) == score);
    }

    test_cond("Insert N elements", daslLength(sl) == (unsigned long)N);
    ...
    for (int i = N - 1; i >= 0; i--) {
        ...
        assert(daslDelete(sl, score, ele) == 1);
        sdsfree(elements[i].ele);
    }

    test_cond("Delete N elements", daslLength(sl) == 0);
